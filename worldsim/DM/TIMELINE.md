# MASTER TIMELINE (DM ground-truth record) — run 2 (engine v2.1)

Loop: write perceptions into KNOWLEDGE → propose (agent THINKS, reads only its files) →
interrogate (no hindsight/future-inventions; force coherence) → defend/revise → adjudicate
(pre-divergence = real-timeline behaviour AND outcomes; tech forward/emergent only) → verify → write.

## 260 BC — world start
- India: Ashoka has conquered Kalinga and does NOT renounce war; the militarist Maurya continues.
- Egypt: Ptolemy II at peak wealth; Alexandria supreme; contesting Syria with the Seleucids.
- China: Qin ascendant in the late Warring States.
- Rome: mid First Punic War vs Carthage.

<!-- Append resolved turns below: TRUE outcomes + fiscal/tech/pop deltas. -->

## 260–235 BC — Turn 1 (engine v2.1; agents thought-first, stayed in-period, fixed incoherences)
Pre-divergence Rome/Egypt/China = OTL behaviour AND outcomes; India (diverged) free & grounded.

- **ROME (OTL).** Pressed the naval war as it really did — corvus boarding, drilling green crews,
  seasoning its best for open-water interdiction off Sicily. Won the First Punic War (~241), took
  Sicily, then Sardinia/Corsica (~238). Suffered the historical 255/253 BC storm catastrophes (huge
  fleet losses) — absorbed via deep manpower + the standardized-hull rebuild it had set up. Veteran
  land-grants drawn (per its own correction) from Carthaginian/Italian public land, NOT allied
  Syracuse. Fiscal: war deficit → relieved by the Carthaginian indemnity + Sicilian revenue.
- **EGYPT (OTL).** Ptolemy II → Ptolemy III (246). Did well in the Third Syrian War (246–241) —
  booty, Levantine standing; held Cyprus/the Levant (its timber/naval lifeline). Formalized a modest
  Meroë trade-treaty (luxuries + grain + temple-recognition for an iron quota + Red Sea elephant-hunt
  passage) — Egypt is slightly less iron-import-fragile but STILL an importer (no domestic source,
  correctly). Funded torsion-artillery refinement (for Syrian sieges) and a scholar–shipwright study
  of sail geometry (captains' present complaint of contrary winds) — both slow, banked, present-need.
  Treasury stays in surplus.
- **CHINA / Qin (OTL).** Pressed broken Zhao after Changping and invested Handan — but the siege
  FAILED to a Wei–Chu relief (~257 BC), exactly as its own contingency foresaw; Qin took the pause,
  held its gains, and rebuilt granary reserves. King Zheng acceded ~246 (regency). Off-season corvée
  extended the canals (Zhengguo ~246) → grain base grows. No unification yet.
- **INDIA (diverged — grounded, ambitious).** Consolidated Kalinga the smart way (chosen prince
  INSTALLED as Viceroy with a real base; a small garrison + paid local informants, not the mobile
  army; succession document sealed-but-unpublished — the fight deferred). Pushed SOUTH where elephants
  dominate: Tamil polities pressed toward tribute/clientage; coastal Sri Lanka gave token trade-tribute
  (presence + trade-leverage, not invaded). Began the PHASED wootz re-equip of the infantry (~80–100k/
  yr) and opened a Seleucid trade for broodmares/remounts (slow cavalry improvement; "never advertise
  the conquest direction"). The NW held diplomatically.

### Fiscal/pop snapshot @235 BC
- Egypt ~7.1M, treasury growing (surplus + Syrian booty). Rome ~4M, treasury recovering (indemnity).
  China/Qin states ~40M, grain base up (canals), treasure dented by the Handan setback. India ~30M,
  large outlays (wootz phase-in + cavalry trade) funded by its huge tax base.
- Quality note: every nation deliberated, rejected the planted bad ideas in-period, and corrected its
