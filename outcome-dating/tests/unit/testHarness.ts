/**
 * Shared test scaffolding for Agent D's unit tests (payment/voucher/
 * dateProposal). Mirrors `tests/foundation.test.ts`'s pattern: each test
 * FILE gets its own dedicated Postgres database (created fresh in
 * `before`, dropped in `after`) so the three files never contend over
 * rows even if the test runner executes them concurrently. Per the task's
 * "use your own database" instruction, every db name is namespaced under
 * `odate_agent_d_*`.
 *
...
export async function createUser(db: TestDb, overrides?: { email?: string }): Promise<string> {
  userCounter += 1;
...
export async function createVenue(db: TestDb, overrides?: { active?: boolean }): Promise<string> {
  const { rows } = await db.pool.query<{ id: string }>(
    `INSERT INTO venues (name, address, latitude, longitude, category, active, margin_percent, time_slot_config, redemption_method)
     VALUES ('Test Cafe', '1 Test St', 39.0, -89.0, 'coffee', $1, 15, '{"slots":[]}'::jsonb, 'qr_scan')
     RETURNING id`,
