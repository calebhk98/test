...
|---|---|---|---|---|
| `auth.service` | A | `register`, `login`, `logout`, `refresh`, `forgotPassword`, `resetPassword`, `verifyAccessToken` | §5, §24.1, §28.1-2 | none (leaf; HTTP layer calls it) |
...
| `filter.service` | B | `getMyFilters`, `updateMyFilters`, `evaluateFilter` (pure), `passesMutualFilters`, `countUsersMatchingMyFilters`, `countUsersWhoseFiltersIMatch` | §9, §24.4 | none (leaf) |
| `compatibility.service` | B | `computePairScore` (pure), `getScore`, `getScoresForCandidates`, `refreshScoresForUser`, `refreshAllScores` | §16, §25.4 | none (leaf; reads `answers`/`questions` directly) |
...
| `textscan.service` | C | `scanText` (pure), `PATTERN_GROUPS` (const) | §12.4, §18.2, §19.3-4 | none (leaf, no I/O) |
| `notification.service` | C | `notify`, `listMyNotifications`, `markNotificationRead`, `deliverPending`, `NOTIFICATION_TEMPLATES` (const) | §20 | none (leaf; every module calls *into* this one) |
| `venue.service` | D | `listActiveVenues`, `getVenue`, `listAvailableTimeSlots`, `adminListVenues`, `adminCreateVenue`, `adminUpdateVenue` | §13.2, §24.8, §27, §30.6 | none (leaf) |
...
| `payments/processor.port` + `fake.processor` + `stripe.processor` | *(shared infra, implemented, not stubbed)* | `PaymentProcessor.{authorize,capture,cancel,refund}` | §14, §28.4 | none |
| `media/moderation.port` + `stub.adapter` | *(shared infra, implemented, not stubbed)* | `ImageModerationPort.analyzePhoto` | §7.2, §18.2 | none |
