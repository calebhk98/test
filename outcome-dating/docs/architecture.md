...
Add the event to `NotificationEventType` (`src/domain/types.ts`), then exactly one static, versioned entry (`snake_case_v1`) to `NOTIFICATION_TEMPLATES` (`src/services/notification.service.ts`); `Record<NotificationEventType, string>` makes a missing entry a `tsc` error. Call `notification.notify(ctx, { userId, eventType, channel, payload })` from wherever the event happens, `payload` may only carry structured data, never a `body`/`text`/`message`/`html` key (see the "no generated prose" invariant above). If the event should also go over push/email/SMS, also call `services/notifications/index.ts#enqueueNotification`, the separate outbox-and-delivery pipeline with its own preferences/quiet-hours/retry logic; `notify` alone only writes the in-app row. Extend `tests/unit/notification.test.ts`'s registry-shape assertions (a new entry is checked for free) and add a call-site test.
...
Register it in `src/jobs/registry.ts`'s `ALL_JOBS` array; `JobScheduler#start` and the `jobs:run`/`jobs:start` CLI both resolve jobs from that one registry, so nothing else needs wiring. Make the underlying service function idempotent (status-guarded `UPDATE`s), a skipped or re-overlapping tick (the advisory-lock guard skips one automatically) must be a safe no-op. Test by calling the job function directly against a `ManualClock`-driven `Ctx`, never by waiting on a real timer.

### Add a payment adapter

`src/services/payments/processor.port.ts#PaymentProcessor` is the interface (`authorize`/`capture`/`cancel`/`refund`). `stripe.processor.ts` is the reference stub; every method's JSDoc is the exact contract a real implementation must satisfy, written so implementing it is filling in a body, not re-deriving the contract from the spec.

To wire in a different real processor: implement the port, extend `src/config/adapters.ts#selectPaymentProcessor`/`describePayments` the same way the `stripe` branch works, add the provider to `env.ts`'s `PAYMENT_PROCESSOR` enum, and add a case to both switch statements (`assertNever` forces every case to be handled, nothing falls through silently). Never construct a concrete adapter anywhere outside `src/config/adapters.ts` and test harnesses; everything else reaches it through `ctx.payments`.

The media-moderation port instead uses an open registry (`registerMediaModerationProvider(name, factory)`) rather than a hardcoded switch, since no real adapter exists yet: a future adapter module registers itself at import time, with no change to `adapters.ts`. Either pattern is acceptable for a new capability; match whichever one it already uses.
