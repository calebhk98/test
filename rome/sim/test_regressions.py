...
# DIFFICULTY MODES (PLAYER REQUEST: "Challenge/Standard/Relaxed/Endless").
# The horizon question became a named-mode menu, with "an exact number of
# years" as one more choice on it (5, here) rather than the only one - see
# cli.py's HORIZON_MODES. 321 matches none of the four named presets, so it
# is typed through that fifth option, exactly as a player asking for a
# number that is not one of the named ones would.
...
check("the difficulty menu, for a civilisation this has been measured for "
      "(Rome), states the dice-free floor by name and says which named "
      "modes fall short of it - the honest warning a mode menu can give, "
      "not four bare numbers",
      "1,017 years" in _hmode1.stdout or "1017 years" in _hmode1.stdout,
      _hmode1.stdout[:4000])
_hmode2 = subprocess.run([sys.executable, os.path.join(HERE, "simulator.py")],
                         input="1\n3\ny\n\nn\n\nquit\n", capture_output=True,
                         text=True, timeout=60, env=_hmode_env)
check("...and for a civilisation that has NOT been measured the same way "
      "(Norse), says so plainly rather than inventing a number for it",
      "has not been measured the same way" in _hmode2.stdout,
      _hmode2.stdout[:4000])
...
check("'saves', typed bare mid-game, lists the save directory without "
      ...
      and "toward the transistor" in _sess1.stdout,
      _sess1.stdout[-1500:])
