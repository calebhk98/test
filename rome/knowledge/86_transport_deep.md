...
**Kernel.** The whole problem of a train is starting it, since you cannot accelerate every wagon at once, so slack in every coupling is a feature: it lets the locomotive take up each wagon's inertia one at a time, "snatching" the train into motion link by link. A screw coupling is tightened by a man walking between wagons turning a threaded rod by hand, slow and historically one of the most dangerous railway jobs, a screwman crushed between buffers is a grim routine record. Sprung buffers cushion the impact, tuned so they neither transmit the shock nor rebound enough to uncouple. A knuckle coupler removes the human: spring-loaded jaws latch automatically on contact, needing accurate, repeatable casting.
...
**Kernel.** A slide valve rubs a flat face continuously under steam, wasting power to friction; a piston valve seals with rings instead, only the rings touch the bore, cutting friction sharply, like the main piston. Stephenson link motion uses two eccentrics, forward and reverse, driving a curved link; sliding the pickup point blends between them, choosing direction and a shortened, efficient "cutoff." Walschaerts gear takes motion from the piston rod's crosshead instead, smoother, easier to maintain, why it displaced Stephenson gear. Compound expansion sends high-pressure steam through small cylinders first, then larger low-pressure ones for a second expansion, more work per unit of coal. An articulated locomotive splits a too-long wheelbase with a centre pivot, two cylinder sets sharing one boiler through flexible pipes that must survive constant flexing.
...
**Kernel.** Clinker planking (each plank overlapping the one below) is not just an old look, the overlap gives real flex and a hydrodynamic benefit at speed. Carvel planking (flush edges over an internal frame) gives a smoother, stronger hull for large vessels, but only if every seam is caulked with oakum, tarred rope fibre driven in from both sides, or it weeps. Frame-first construction (ribcage first, then planking) gives far more precise, repeatable shapes than the older shell-first method. A keelson, bolted along the keel over the frames, stops "hogging," the slow sag of a hull's ends, which cracks a ship's back without it. Copper sheathing is sacrificial, dissolving to poison marine growth, why iron nails near it corrode fast and must themselves be copper.
...
**Kernel.** An iron hull is a genuine step change: iron plate can be built larger and thinner-walled than wood, which needs thickness against rot and worm, so an iron ship can be much bigger for the same displacement. Riveted plating, red-hot rivets hammered flush, is labour-intensive but reliable if well done; a poor rivet leaks slowly, catastrophically once corrosion widens the gap. Welded seams are stronger and cheaper in labour but riskier: a failing rivet seam leaks and warns you, a welded seam with hidden stress cracks suddenly with no warning, so weld design must avoid sharp corners. A double bottom roughly doubles cost but keeps the ship afloat if the outer hull is holed. Watertight bulkheads only work if every penetration is sealed; one careless open door defeats the whole system.
...
**Kernel.** See 92_vehicles_flight.md's sternpost_rudder entry: a single centred blade gives far more steering leverage than side-hung oars, letting a captain point high into the wind, tack, and choose a departure date. A paddle wheel throws water up as well as back, wasting energy, worse in rough seas. A screw propeller stays submerged regardless of roll and is more efficient, but needs precise shaft alignment, or vibration wears the bearings. Variable pitch reverses thrust without reversing the engine, since reversing a large engine's rotation is slow. Reduction gearing steps a turbine's speed down and torque up to usable propeller speed; teeth must be precisely cut and hardened or the gearing eats the efficiency gain. A stern tube seals where the shaft exits the hull, one of the more failure-prone points on a ship.
...
**Kernel.** A windlass or capstan multiplies human force through gear reduction; without it no crew can haul a large anchor against water resistance and its own weight, gearing is the difference between possible and impossible. Chain cable does not rot or chafe on a rocky bottom, but every link must be forge-welded fully shut, one flawed link opens under load and the ship is adrift. A stockless anchor's pivoting flukes stow flat, no rigging needed to haul and secure. A bilge pump exists because a ship always leaks, pumping in relays is the accepted cost, not a sign of failure. Ballast tanks trim and stabilise with pumped seawater, but uneven ballast is as dangerous as loose cargo. Block and tackle gives advantage proportional to rope parts, but each added pulley adds its own friction, so very large tackles lose efficiency to their own hardware.
...
**Kernel.** See 92_vehicles_flight.md's marine_chronometer entry: longitude is a clock problem, and a spring-driven timepiece that loses time to temperature-driven expansion is useless until compensated. A sextant reads the angle between a body and horizon via two mirrors, one half-silvered; accuracy depends on mirror flatness and silvering (30_glass_optics.md), a manufacturing bottleneck, not conceptual. A gyrocompass repeater is a dial slaved electrically to one master compass, so many stations read true heading without each needing its own spinning mechanism. A log and sounding line are almost embarrassingly simple, why they lasted so long. A Fresnel lens steps a lens into concentric rings instead of one curve, equal focusing power at a fraction of the glass weight. A ship's telegraph replaces shouted orders with a dial and bell, since the engine room cannot hear the bridge.
