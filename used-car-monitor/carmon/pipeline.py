...
def run_daily(
    ...
    The steps below run in a fixed order — fetch, sweep, filter, enrich, score/upsert,
    market — each one a named helper so this function reads as that pipeline rather than
    as one long loop nest.
    """
    ...
    if not dry_run:
        _clear_demo_before_run(conn, config, result)
    ...
    try:
        ...
        raw_listings = _fetch_raw_listings(config, conn, client, run_date, result)
        candidates = _select_candidates(raw_listings, config.search, result)
        _run_enrichment(config, conn, candidates, result)
        _score_and_store(config, conn, candidates, run_date, dry_run, result)

        if not dry_run:
            db.mark_inactive_before(conn, run_date)
            _refresh_market(config, conn, result)

    except (MarketCheckError, QuotaExceeded) as exc:
        result.errors.append(str(exc))
        LOG.error("Run failed: %s", exc)
    finally:
        _finish_run(conn, run_id, client, result, owns_conn)
    return result


def _clear_demo_before_run(conn: sqlite3.Connection, config: Config, result: RunResult) -> None:
...
def _fetch_raw_listings(
    config: Config, conn: sqlite3.Connection, client: MarketCheckClient, run_date: str, result: RunResult
) -> List[Dict[str, Any]]:
    """The two MarketCheck passes (plain + certified) plus the month-end sweep, combined."""
...
def _record_filtered(result: RunResult, reason: str) -> None:
    result.filter_reasons[reason] = result.filter_reasons.get(reason, 0) + 1
    result.filtered_out += 1


def _select_candidates(
    raw_listings: List[Dict[str, Any]], search: Dict[str, Any], result: RunResult
) -> List[Dict[str, Any]]:
    """Normalize, dedupe by VIN, and drop anything `filter_reason` rejects."""
    ...
    for raw in raw_listings:
        ...
        if not listing:
            _record_filtered(result, "missing VIN")
            continue
        if listing["vin"] in seen_vins:
            continue
        reason = filter_reason(listing, search)
        if reason:
            _record_filtered(result, reason)
            continue
...
def _run_enrichment(
    config: Config, conn: sqlite3.Connection, candidates: List[Dict[str, Any]], result: RunResult
) -> None:
...
def _score_and_store(
    ...
    """Score every candidate and, unless this is a dry run, upsert it."""
...
def _refresh_market(config: Config, conn: sqlite3.Connection, result: RunResult) -> None:
    """Market comparison runs last: it needs every listing from this run in place before it
    can say what "the market" looks like."""
    try:
...
def _finish_run(
    conn: sqlite3.Connection,
    run_id: int,
    client: Optional[MarketCheckClient],
    result: RunResult,
    owns_conn: bool,
) -> None:
...
# --- optional scrapers ----------------------------------------------------
# Running the scraper adapters and handling per-model reference sources is a separate
# concern from the MarketCheck daily run above; that code lives in `scrape_runner.py`.
# Re-exported here (a deferred import, since `scrape_runner` imports back from this module)
# so `from .pipeline import run_scrapers` etc. keeps working for every existing caller.
from .scrape_runner import (  # noqa: E402  (must follow this module's own definitions)
    probe_scrapers,
    reference_models,
    run_scrapers,
)
