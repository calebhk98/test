# V2 Historical Impact Reference — *The Long Way Home*
...
[DIV] By the Second Dacian War, Daniel has also introduced crossbow spanning levers and bolt standardization to the army — Celer's unit was among the first equipped. Crossbows matter in a balloon basket specifically: a regular bow requires full arm extension and a standing draw in a confined, swaying platform. A pre-spanned crossbow requires only aim and trigger pull. Two crossbowmen with pre-loaded bolts in a balloon basket are a more practical weapons platform than two archers. The balloon rigs at Sarmizegetusa carry crossbowmen, not archers. This detail is not dramatized in detail — one line noting the distinction is enough — but it is the accurate version of events.
...
[DIV] By 113, Daniel has: manned balloon crossbowmen (now a standard formation attachment, two balloon rigs per legion on campaign — crossbows specified over bows for basket use by 108 AD), logistics accounting in decimal notation (reducing supply waste by an estimated 15%), rag paper enabling fast duplication of maps and orders, and improved siege engineering through his ongoing relationship with Apollodorus.
...
## APPENDIX: DIVERGENCE VISIBILITY TIMELINE

For each chapter's year, a writer can quickly check which changes would be perceptible to an on-page character:
