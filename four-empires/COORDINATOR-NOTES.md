...
**An agent's report that a fix was applied is not evidence that it was applied.** Egypt reported six fixes complete and clean; two of them had never landed in the file, and it found this only because it opened the file on the following round instead of trusting its own previous edit. The reports are not dishonest — the agent believes the edit went in. But an edit that silently fails produces exactly the same report as one that succeeds, and a coordinator reading reports rather than files will carry the failure forward through every subsequent round, including into the passes that certify the document clean. **Run a mechanical presence check over the specific claimed text before treating any round as closed.** It is cheap, it needs no judgement, and it is the only thing that distinguishes a fix from a report of a fix.
